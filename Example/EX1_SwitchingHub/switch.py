...
class Switch(app_manager.RyuApp):
    ...
    # The controller sends a features request message to the switch. 
    @set_ev_cls(ofp_event.EventOFPSwitchFeatures, CONFIG_DISPATCHER)
    def switch_features_handler(self, ev: OFPSwitchFeatures):
        ...
        match = parser.OFPMatch() # 可以match任何封包
        actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,
                                          ofproto.OFPCML_NO_BUFFER)] # send to controller
        self.add_flow(datapath, 0, match, actions) # 優先度設最低

    @set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)
    def _packet_in_handler(self, ev: OFPPacketIn):
